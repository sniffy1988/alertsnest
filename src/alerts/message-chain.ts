...
/** «На даний час без повторних пусків КАБ» — stand-down, not a new threat. */
export function isNoMoreLaunches(text: string): boolean {
  const n = foldUa(text);
  return (
    /без\s+повторн|без\s+нових\s+пуск|без\s+новых\s+пуск|пуски\s+припинен|пуски\s+прекрат|нема(є)?\s+повторн|нет\s+повторн|новых\s+пусков\s+нет|повторних\s+пусків\s+нема|наразі\s+без\s+пуск|на\s+даний\s+час\s+без|в\s+данный\s+момент\s+без|пока\s+без\s+пуск|поки\s+без\s+пуск/.test(
      n,
    )
  );
}
...
const ALL_CLEAR_RE =
  /(видбий|відбій|отбой|укриття знят|укрытие снят|все чисто|усе чисто|повітря чисто|воздух чисто|тривог[уи] скасован|тревог[уи] отмен|тривог\w*\s+зня|тревог\w*\s+снят|\bзнято\b|\bснята\b|\bснято\b)/;
