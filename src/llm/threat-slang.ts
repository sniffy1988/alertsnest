...
export function isThreatLabel(name: string): boolean {
  ...
  if (
    /^(балист\w*|баллист\w*|шахед\w*|шаболд\w*|мопед\w*|геран\w*|молні\w*|молни\w*|кинжал\w*|кинджал\w*|орешник\w*|калибр\w*|калібр\w*|іскандер\w*|искандер\w*|ракет\w*|каб\w*|фаб\w*|умпк|бнр|бпла\w*|ппо|пво|тривог\w*|тревог\w*|загроз\w*|угроз\w*|повітря|воздух|приліт\w*|прилет\w*|вибух\w*|взрыв\w*|бандерол\w*|oreshnik|molniya|molnia|shahed|kinzhal|iskander)$/.test(
      n,
    )
  ) {
    return true;
